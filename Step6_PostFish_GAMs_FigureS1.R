...
bmb_gam = bmb %>%
  filter(doy>128) %>% #subset to only the post-fish addition period
...
#Chlorophyll GAM
chl_gam <- gam(chl ~ treatment + s(doy, k = 36) + s(doy, by = fishtreat), 
...
text(150, 5, "edf = 3.376***", col = "gray40", cex = 1.2, font = 4)
...
text(150, 5, "edf = 1.331", col = low2, cex = 1.2, font = 1) #summary() results
text(150, 4, "edf = 7.563***", col = high2, cex = 1.2, font = 4) #summary() results

#parametric effects plot
plot(c(1,2,3), c(0, -0.04225, 1.43688), 
     col = c(no2, low2, high2), pch = 15, cex = 2,
     xlim = c(0.5,3.5), ylim = c(-0.5,2), xaxt = "n", cex.axis = 1.2)
axis(1, at = c(1,2,3), labels = c("Ref","Harv","Amb"), cex.axis = 1.2)
#connect the reference point to the two other treatments
lines(c(1,2), c(0,-0.04225), lty = 2)
lines(c(1,3), c(0, 1.43688), lty = 2)
points(c(1,2,3), c(0, -0.04225, 1.43688), 
       col = c(no2, low2, high2), pch = 15, cex = 2)
#Plot the error terms to the parametric estimates
lines(c(2,2), c(-0.04225+0.13526, -0.04225-0.13526), col = low2, lwd = 2)
text(2, -0.04225+0.13526, "", cex = 2)
lines(c(3,3), c(1.43688+0.13526, 1.43688-0.13526), col = high2, lwd = 2)
text(3, 1.43688+0.13526, "**", cex = 2)
...
plot(zoop_gam, select = 3,  
     ...
     main = "", xlab = "", ylab = "",
     xaxt = "n", yaxt = "n")
text(150, log10(7500), "edf = 1.000", col = low2, cex = 1.2, font = 1)
text(150, log10(4000), "edf = 3.452***", col = high2, cex = 1.2, font = 4)

plot(c(1,2,3), c(0, -0.36950, -0.39517), #ref, low, high treatments
     col = c(no2, low2, high2), pch = 15, cex = 2, cex.axis = 1.2,
     xlim = c(0.5,3.5), ylim = c(-0.5, 0.05), xaxt = "n")
axis(1, at = c(1,2,3), labels = c("Ref","Harv","Amb"), cex.axis = 1.2)
lines(c(1,2), c(0,-0.36950), lty = 2)
lines(c(1,3), c(0, -0.39517), lty = 2)
points(c(1,2,3), c(0, -0.36950, -0.39517), 
       col = c(no2, low2, high2), pch = 15, cex = 2)
lines(c(2,2), c(-0.36950+0.12146, -0.36950-0.12146), col = low2, lwd = 2)
text(2, -0.36950+0.12146, "**", cex = 2)
lines(c(3,3), c(-0.39517+0.12365, -0.39517-0.12365), col = high2, lwd = 2)
text(3, -0.39517+0.12365, "**", cex = 2)
...
plot(length_gam, select = 1, 
     ...
     shade.col=no1, cex = 0, cex.axis= 1.2,
     xlab = "", ylab = "")
text(150, 160, "edf = 1.533***", col = "gray40", cex = 1.2, font = 4)
...
plot(length_gam, select = 3,  
     ...
     xaxt = "n", yaxt = "n")
mtext(side = 1, line = 3, "Day of Year")
text(150, 160, "edf = 2.356**", col = low2, cex = 1.2, font = 4)
text(150, 136, "edf = 1.001***", col = high2, cex = 1.2, font = 4)

plot(c(1,2,3), c(0, -15.639, -25.723), 
     col = c(no2, low2, high2), pch = 15, cex = 2, cex.axis = 1.2,
     xlim = c(0.5,3.5), ylim = c(-30,2), xaxt="n")
axis(1, at=c(1,2,3), labels = c("Ref", "Harv", "Amb"), cex.axis = 1.2)
lines(c(1,2), c(0,-15.639), lty = 2)
lines(c(1,3), c(0, -25.723), lty = 2)
points(c(1,2,3), c(0, -15.639, -25.723), 
       col = c(no2, low2, high2), pch = 15, cex = 2)
lines(c(2,2), c(-15.639+4.821, -15.639-4.821), col = low2, lwd = 2)
text(2, -15.639+4.821, "**", cex = 2)
lines(c(3,3), c(-25.723+4.730, -25.723-4.730), col = high2, lwd = 2)
text(3, -25.723+4.730, "***", cex = 2)
